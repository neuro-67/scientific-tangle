...
For this project, follow the project convention first:

- `entities/session/` owns sign-in/session API, session queries, and session
  context.
- `entities/user/` owns reusable user and role domain types.
- `app/router/guards/` owns route guards such as `auth-guard.tsx` and
  `guest-guard.tsx`.
- `shared` should contain only pure token/HTTP infrastructure if needed, not
  the session domain itself.

### Auth/session data: prefer `entities/session` for this project family

In minimal FSD projects, token storage and tiny session helpers can be treated
as infrastructure and kept in Shared:
...
The `shared/auth/index.ts` re-exports from these files following the standard
public API pattern. In this project, however, prefer an entity when the session
is part of app-wide domain state:
...
entities/session/
  ...
  model/
    session.types.ts
    session.context.tsx
  index.ts             ← export * as sessionApi from "./api"

entities/user/
  model/user.types.ts  ← User, Role, permissions-related domain types
  index.ts
...
### When to use shared/auth vs session/user entities
...
`shared/auth` is the simpler default for small/minimal projects. For this
project, prefer `entities/session` for the session/auth domain and
`entities/user` for reusable user/role types.
...
// Path B: mature project split
entities/session/              ← Current session/auth API/context
entities/user/                 ← Generic user and role domain types
```
...
| Types for a specific entity's domain model | `entities/<name>/model/<name>.types.ts` |
| Types used only within one page | `pages/<role>/<name>/model/<name>.types.ts` |
| Types used only within one feature | `features/<name>/model/<name>.types.ts` |
| Generic utility types (e.g., `Nullable<T>`) | Domain-named files in `shared/lib/` (e.g., `shared/lib/nullable.ts`) |
| Generic response/pagination/React utility types | `shared/types/` |

Per Rule 4-4 (domain-based file naming), avoid grouping all types in
`types.ts` or `utils.ts`. Domain-prefixed files such as `order.types.ts` and
`api.types.ts` are allowed because they answer "types for what?".
...
```typescript
// entities/product/model/product.types.ts: domain model layered on top
...
### Entity API namespaces

For stable domains, prefer the project entity API layout. Each request is a
separate file, the `api/index.ts` file exports request functions and the
`queries` factory, and the slice `index.ts` exposes the whole API as a
namespace:
...
export { openDispute } from "./open-dispute";
export { queries } from "./order.queries";
...
Consumers then import from the slice public API:
...
For this project, Option 3 is the default for stable migrated domains.
...
```typescript
// src/entities/order/api/order.queries.ts
import { queryOptions } from "@tanstack/react-query";
import { getOrder, type Req as OrderReq } from "./get-order";
import { getOrders, type Req as GetOrdersReq } from "./get-orders";

export const queries = {
  all: () => ["order"],
  lists: () => [...queries.all(), "list"],
  orderList: (query: GetOrdersReq) => queryOptions({
    queryKey: [...queries.lists(), query],
    queryFn: () => getOrders(query),
  }),
  byId: (query: OrderReq) => queryOptions({
    queryKey: [...queries.all(), query.id],
    queryFn: () => getOrder(query),
  }),
};
```

Consume with `useQuery(orderApi.queries.byId({ id }))`. For pagination,
...
(`queryClient.invalidateQueries({ queryKey: orderApi.queries.all() })`) without
...
For this project style, prefer `entities/<name>/api/<name>.queries.ts` and
export it as `queries` through the entity API namespace.
