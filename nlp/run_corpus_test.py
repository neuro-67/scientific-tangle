...
6. Если в тексте указан автор/докладчик — создавай Expert и связывай Publication -> [authored_by] -> Expert, а вывод -> [validated_by] -> Expert, если именно этот автор сделал измерение/вывод. ЗАПРЕЩЕНО создавать узел Expert без единой связи: КАЖДЫЙ Expert ОБЯЗАН иметь хотя бы одно ребро (authored_by от Publication, либо expert_in к Topic, либо validated_by от Finding). Если ты упоминаешь автора из списка литературы — обязательно создай и узел Publication этой ссылки, и ребро Publication -> [authored_by] -> Expert; изолированные авторы без связей недопустимы.
